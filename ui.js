  ...
  document.querySelectorAll('.track-row').forEach(row=>{
    ...
    attachLongPress(row, (x,y)=>{
      ...
      openContextMenu(x, y, [
        ...
        {label:'Çalma Listesine Ekle', onSelect:()=> openChoosePlaylistsSheet(id)},
        {label:'Çoklu Seçim'},
        {label:'Şarkıyı Sil'},
      ]);
    });
...
function openSheet(html, opts={}){
  ...
  sheetEl.innerHTML = `<div class="sheet-drag-zone"><div class="sheet-grip"></div><button class="sheet-close" id="sheetCloseBtn" aria-label="Kapat"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6 6 18"/></svg></button></div>` + html;
  overlay.classList.add('show');
  document.getElementById('sheetCloseBtn').addEventListener('click', closeSheet);
  enableSheetSwipeClose();
}
function enableSheetSwipeClose(){
  const zone=sheetEl.querySelector('.sheet-drag-zone');
  if(!zone) return;
...
function openTrackEditSheet(draft, isNew){
  ...
  openSheet(`
    ...
    <div class="sheet-scroll">
      ...
      <div class="field"><label>Şarkı Sözleri</label><div class="ta-wrap"><textarea id="fLyrics" maxlength="1000" rows="12" placeholder="Henüz şarkı sözü eklenmedi.">${escapeHtml(draft.lyrics)}</textarea><span class="ta-count" id="fLyricsCount">0/1000</span></div></div>
    </div>
